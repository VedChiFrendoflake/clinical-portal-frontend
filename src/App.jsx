...
const EncounterVoiceNote = ({ targetPatient, visitDate, providerName, noteValue, setNoteValue, showToast }) => {
  ...
  const uploadVoiceNote = async (audioBlob) => {
    ...
    try {
      ...
      if (data.status === "success") { setNoteValue(data.note); showToast("Perfectly formatted!", "success"); }
...
export default function App() {
  ...
  // ☢️ STATE FOR RADIOLOGY UPLOAD FORM
  ...
  // ☢️ SUBMIT FUNCTION FOR RADIOLOGY SCANS
  const handleRadiologyPacketSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) { showToast("Radiology successfully synced and layman explanation generated!", "success"); fetchPatientData(activePatient); setRadioUploadAsset({ title: '', note: '', file: null }); }
  ...
  const handleAddOrder = async (e) => {
    e.preventDefault();
    if (!orderInput.test_name) return showToast("Provide diagnostic test name.", "error");
